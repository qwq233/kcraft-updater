...
// Class net.kcraft.generator.Init
//--------------------------------------------------
...
import net.kcraft.generator.BuildConfig;
...
class Init {
    static void initSystem(String[] args) {
        try {
            ...
            File basekcraftDir = Environment.getBasekcraftDir();

            if (!basekcraftDir.exists()) {
                try {
                    Environment.getLogger().log(Level.FINE,
                            "KCraft root directory not found on '" + basekcraftDir.getCanonicalPath() + "'. Create.");
                    FileUtils.forceMkdir(basekcraftDir);
                } catch (Exception ex) {
                    Environment.getLogger().log(Level.SEVERE,
                            "Create KCraft root directory FAILED '" + basekcraftDir.getCanonicalPath() + "'.");
                    System.exit(9);
                }
    ...
    static CommandLine getCmd(String[] args) throws ParseException {
        ...
        ops.addOption("p", "path", true, "Path to KCraft root directory. Default ./kcraft");
