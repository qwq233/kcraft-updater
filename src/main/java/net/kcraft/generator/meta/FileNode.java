...
public final class FileNode implements Cloneable {
    ...
    public String getRelativePath() {
        if (relativePath == null) {
            if (file.exists()) {
                try {
                    relativePath = FileTool.getRelativePath(Environment.getBasekcraftPath(), file.getCanonicalPath());
                } catch (IOException ex) {
                    relativePath = null;
                }
            }
            if (relativePath == null)
                relativePath = FileTool.getRelativePath(Environment.getBasekcraftPath(), file.getPath());
        }
        return relativePath;
    }
